...
* ``MicromorphicRVESolver`` (ROM, default),
* ``MicromorphicHyperelasticHomogenizationSolver`` (FOM, set USE_ROM=False).

Prerequisites (ROM):
    1. Run ``run_micromorphic.py`` with ``save_modes=True`` — produces
       ``output/snapshots/phi_*.npy``.
    2. Run ``build_rom.py`` — produces ``ecm/`` with POD+ECM data.

Prerequisites (FOM):
    Same step 1 only (phi modes must exist in ``output/snapshots/``).
...
from fe2_rom.macro_solver.material_micromorphic import MicromorphicRVEMaterial
from fe2_rom.macro_solver.macro_micromorphic import MacroMicromorphicSolver

# ---------------------------------------------------------------------------
# Configuration
# ---------------------------------------------------------------------------
USE_ROM    = True     # True → MicromorphicRVESolver, False → FOM homogenization
...
ROM_DIR  = os.path.join(HERE, "ecm")
PHI_DIR  = os.path.join(HERE, "output", "snapshots")
...
if USE_ROM:
    ...
    from fe2_rom.hyperelastic_solver import MicromorphicHyperelasticHomogenizationSolver

    def rve_factory(rank: int, index: int):
        out_dir = os.path.join(HERE, "output_macro_rve", f"rve_{rank}_{index}")
        rve = MicromorphicHyperelasticHomogenizationSolver(
            mesh_path=RVE_MESH,
            comm=MPI.COMM_SELF,
            gdim=2,
            material=NeoHookean(mu=MU_MICRO, lmbda=LAM_MICRO),
...
output_dir = os.path.join(HERE, f"output_macro_micromorphic_{mode_label}")
